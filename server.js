...
async function getBatchSummary(batch) {
  ...
  const accountDisplay = String(first.customer_telegram || first.customer_telegram_username || "").trim() || "Tasdiqlanmagan";
...
app.post("/order/:id", async (req, res, next) => { const client = await pool.connect(); try {
  ...
  const customerTelegramUsername = String(customerTelegram || "");
