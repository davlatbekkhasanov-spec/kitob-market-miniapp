...
app.get("/cart", async (req, res, next) => { try {
  ...
  res.send(page("Savatcha", `<div class="panel" style="max-width:900px;margin:0 auto"><div class="actions" style="margin-bottom:12px"><a class="btn dark" href="/">← Davom etish</a></div><h2>Savatcha</h2>${sourceBadge(sourceCode)}${body}${items.length ? `<form class="form" method="post" action="/checkout" style="margin-top:16px"><input type="hidden" name="source_code" value="${esc(sourceCode)}" /><div class="grid2"><input name="customer_name" placeholder="Ismingiz" /><input name="phone" placeholder="Telefon raqam" required /></div><div class="grid2"><button type="button" class="btn soft" id="cartLocBtn" onclick="getCartLocation()">📍 Lokatsiyani yuborish</button><div class="small" id="cartLocText">Lokatsiya hali tanlanmadi</div></div><input type="hidden" name="latitude" id="cartLatField" /><input type="hidden" name="longitude" id="cartLngField" /><input type="hidden" name="location_url" id="cartLocationUrlField" /><textarea name="address_text" placeholder="Manzil yoki lokatsiya havolasi"></textarea><div class="order-summary"><div>Mahsulotlar: <b>${money(subtotal)}</b></div><div>Dostavka: <b>${money(items.length ? DELIVERY_FEE : 0)}</b></div><div style="margin-top:8px">Jami: <b>${money(total)}</b></div></div><button class="btn green" type="submit">Bitta zakaz qilish</button></form><script>function setCartLocation(lat,lng,label){document.getElementById('cartLatField').value=lat;document.getElementById('cartLngField').value=lng;document.getElementById('cartLocationUrlField').value='https://maps.google.com/?q='+lat+','+lng;try{localStorage.setItem('last_location_lat',String(lat));localStorage.setItem('last_location_lng',String(lng));}catch(_e){}document.getElementById('cartLocText').textContent=label||'✅ Lokatsiya olindi';document.getElementById('cartLocBtn').textContent='✅ Tayyor';}function applySavedCartLocation(){try{const lat=localStorage.getItem('last_location_lat');const lng=localStorage.getItem('last_location_lng');if(lat&&lng){setCartLocation(lat,lng,"✅ Oxirgi lokatsiya qo'llandi");return true;}}catch(_e){}return false;}async function applyCartIpLocation(){const resp=await fetch('https://ipapi.co/json/');if(!resp.ok) throw new Error('ip');const data=await resp.json();if(!data||!data.latitude||!data.longitude) throw new Error('ip');setCartLocation(data.latitude,data.longitude,'✅ Lokatsiya olindi (taxminiy)');}function getCartLocation(){const btn=document.getElementById('cartLocBtn');const text=document.getElementById('cartLocText');if(!navigator.geolocation){text.textContent="Geolokatsiya yo'q. Lokatsiya taxminiy olinmoqda...";btn.disabled=true;btn.textContent='⏳ Lokatsiya olinmoqda...';applyCartIpLocation().catch(function(){btn.disabled=false;btn.textContent='📍 Lokatsiyani yuborish';text.textContent="Lokatsiyani avtomatik olish bo'lmadi, manzilni yozing";});return;}btn.disabled=true;btn.textContent='⏳ Lokatsiya olinmoqda...';navigator.geolocation.getCurrentPosition(function(pos){setCartLocation(pos.coords.latitude,pos.coords.longitude);},function(){navigator.geolocation.getCurrentPosition(function(pos){setCartLocation(pos.coords.latitude,pos.coords.longitude);},function(){if(applySavedCartLocation()) return;applyCartIpLocation().catch(function(){btn.disabled=false;btn.textContent='📍 Lokatsiyani yuborish';text.textContent="Lokatsiyani avtomatik olish bo'lmadi, manzilni yozing";});},{enableHighAccuracy:true,timeout:30000,maximumAge:0});},{enableHighAccuracy:false,timeout:20000,maximumAge:60000});}</script>` : ""}</div>`, { admin:isAdmin(req) }));
  res.send(page("Savatcha", `<div class="panel" style="max-width:900px;margin:0 auto"><div class="actions" style="margin-bottom:12px"><a class="btn dark" href="/">← Davom etish</a></div><h2>Savatcha</h2>${sourceBadge(sourceCode)}${body}${items.length ? `<form class="form" method="post" action="/checkout" style="margin-top:16px"><input type="hidden" name="source_code" value="${esc(sourceCode)}" /><div class="grid2"><input name="customer_name" placeholder="Ismingiz" /><input name="phone" placeholder="Telefon raqam" required /></div><div class="grid2"><button type="button" class="btn soft" id="cartLocBtn" onclick="getCartLocation()">📍 Lokatsiyani yuborish</button><div class="small" id="cartLocText">Lokatsiya hali tanlanmadi</div></div><input type="hidden" name="latitude" id="cartLatField" /><input type="hidden" name="longitude" id="cartLngField" /><input type="hidden" name="location_url" id="cartLocationUrlField" /><textarea name="address_text" placeholder="Manzil yoki lokatsiya havolasi"></textarea><div class="order-summary"><div>Mahsulotlar: <b>${money(subtotal)}</b></div><div>Dostavka: <b>${money(items.length ? DELIVERY_FEE : 0)}</b></div><div style="margin-top:8px">Jami: <b>${money(total)}</b></div></div><button class="btn green" type="submit">Bitta zakaz qilish</button></form><script>function setCartLocation(lat,lng){document.getElementById('cartLatField').value=lat;document.getElementById('cartLngField').value=lng;document.getElementById('cartLocationUrlField').value='https://maps.google.com/?q='+lat+','+lng;}function getCartLocation(){const btn=document.getElementById('cartLocBtn');const text=document.getElementById('cartLocText');if(!navigator.geolocation){text.textContent="Geolokatsiya yo'q. Manzilga Google Maps link kiriting.";return;}btn.disabled=true;btn.textContent='⏳ Lokatsiya olinmoqda...';navigator.geolocation.getCurrentPosition(function(pos){setCartLocation(pos.coords.latitude,pos.coords.longitude);text.textContent='✅ Lokatsiya olindi';btn.textContent='✅ Tayyor';},function(firstErr){navigator.geolocation.getCurrentPosition(function(pos){setCartLocation(pos.coords.latitude,pos.coords.longitude);text.textContent='✅ Lokatsiya olindi';btn.textContent='✅ Tayyor';},function(secondErr){btn.disabled=false;btn.textContent='📍 Lokatsiyani yuborish';const reason=(secondErr&&secondErr.message)||(firstErr&&firstErr.message)||'Ruxsat yoki internetni tekshiring';text.textContent='❌ Lokatsiya olinmadi. Manzilga Google Maps link kiriting. ('+reason+')';},{enableHighAccuracy:true,timeout:30000,maximumAge:0});},{enableHighAccuracy:false,timeout:20000,maximumAge:60000});}</script>` : ""}</div>`, { admin:isAdmin(req) }));
} catch (e) { next(e); } });
