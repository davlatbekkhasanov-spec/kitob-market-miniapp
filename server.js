...
function buildTelegramAccountLink(username = "", telegramId = "") {
  const u = String(username || "").trim();
  const id = String(telegramId || "").trim();
  if (u) return `https://t.me/${u.replace(/^@+/, "")}`;
  if (id && /^\d+$/.test(id)) return `tg://user?id=${id}`;
  return "";
}
...
async function sendBatchToGroup(batch) {
  ...
  const accountLink = buildTelegramAccountLink(batch.rows?.[0]?.customer_telegram_username, batch.rows?.[0]?.customer_telegram);
  const buttons = [];
...
async function getBatchSummary(batch) {
  ...
  const accountLink = buildTelegramAccountLink(first.customer_telegram_username, first.customer_telegram);
  const accountLabel = first.customer_telegram_username || (first.customer_telegram ? `ID: ${first.customer_telegram}` : "");
  const accountPart = accountLink ? `\n🆔 Buyurtmachi akkaunt: ${accountLabel}\n🔗 Akkountga o'tish: ${accountLink}` : "";
...
async function updateGroupOrderMessage(batch) {
  ...
  const accountLink = buildTelegramAccountLink(summary.rows?.[0]?.customer_telegram_username, summary.rows?.[0]?.customer_telegram);
