...
if (!process.env.DATABASE_URL) {
  console.error("DATABASE_URL topilmadi. .env yoki Railway Variables ichiga DATABASE_URL kiriting.");
  process.exit(1);
}

if (!process.env.SESSION_SECRET || SESSION_SECRET === "kitob-market-super-secret-2026") {
  console.warn("Ogohlantirish: SESSION_SECRET default qiymatda turibdi. Production uchun albatta almashtiring.");
}

if (!process.env.ADMIN_PIN || ADMIN_PIN === "2026") {
  console.warn("Ogohlantirish: ADMIN_PIN default qiymatda turibdi. Production uchun albatta almashtiring.");
}
...
async function saveImage(file) {
  ...
  if (HAS_CLOUDINARY) {
    try {
      ...
      if (uploaded && uploaded.secure_url) return uploaded.secure_url;
    } catch (_e) {
      console.warn("Cloudinary upload ishlamadi, local fallback ishlatiladi.");
    }
  }
...
app.get("/order/:id", async (req, res, next) => { try {
  ...
  const bindToken = ensureBindToken(req, res);
  const verifyUrl = TELEGRAM_BOT_USERNAME ? `https://t.me/${TELEGRAM_BOT_USERNAME}?start=verify_${bindToken}` : "";
  const verifyBlock = TELEGRAM_BOT_USERNAME ? `<div class="actions"><a class="btn soft" id="verifyBtn" target="_blank" href="${esc(verifyUrl)}">Shaxsingizni tasdiqlash</a><span class="tag" id="verifyStatus" style="display:none">✅ Tasdiqlandi</span></div><div class="small">Chekni Telegram'ga avtomatik yuborish uchun bir marta tasdiqlang.</div>` : `<div class="small">Telegram tasdiqlash uchun TELEGRAM_BOT_USERNAME ni sozlang.</div>`;
...
app.get("/cart", async (req, res, next) => { try {
  ...
  const bindToken = ensureBindToken(req, res);
  const verifyUrl = TELEGRAM_BOT_USERNAME ? `https://t.me/${TELEGRAM_BOT_USERNAME}?start=verify_${bindToken}` : "";
  const verifyBlock = TELEGRAM_BOT_USERNAME ? `<div class="actions"><a class="btn soft" id="verifyBtn" target="_blank" href="${esc(verifyUrl)}">Shaxsingizni tasdiqlash</a><span class="tag" id="verifyStatus" style="display:none">✅ Tasdiqlandi</span></div>` : `<div class="small">Telegram tasdiqlash uchun TELEGRAM_BOT_USERNAME ni sozlang.</div>`;
...
app.get("/health", (_req, res) => {
  res.json({ ok: true, app: "kitob-market", uptime: process.uptime() });
});
...
app.use((err, req, res, _next) => { 
  console.error(err);
  res.status(500).send(page("Xatolik", `<div class="panel" style="max-width:760px;margin:0 auto"><div class="alert err">${esc(err && err.message ? err.message : "Noma'lum xatolik")}</div><a class="btn dark" href="javascript:history.back()">← Ortga</a></div>`, { admin:isAdmin(req) })); 
});
...
(async () => { 
  try { 
    await initDb(); 
    app.listen(PORT, () => console.log(`Kitob Market server listening on port ${PORT}`)); 
  } catch (e) { 
    console.error(e); 
    process.exit(1); 
  } 
