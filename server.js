...
function openWebAppButton(sourceCode = "") {
  ...
  return { text: `📲 Mini Appni ochish${meta.name ? ` (${meta.name})` : ""}`, web_app: { url: `${APP_URL}/?source=${encodeURIComponent(code)}` } };
}
async function sendBatchToGroup(batch) {
  ...
  const accountLink = buildTelegramAccountLink(batch.rows?.[0]?.customer_telegram_username);
  const buttons = [];
...
async function sendAcceptedMessage(target, sourceName = "") {
...
Hurmatli mijoz, buyurtmangiz tizimga qabul qilindi.${sourceLine}
  ...
  try {
    ...
    if (update.message && update.message.text && update.message.chat && update.message.chat.type === "private") {
      ...
      if (startVerifyMatch) {
        ...
        const caption = meta.code
          ? `Assalomu alaykum! ${meta.name} uchun Mini App tayyor. Pastdagi tugma orqali kirib buyurtma bering.`
          : "Assalomu alaykum! Pastdagi tugma orqali Mini Appga kirib buyurtma bering.";
...
app.post("/order/:id", async (req, res, next) => { const client = await pool.connect(); try {
  ...
  await sendAcceptedMessage(customerTelegram, meta.name);
