...
const PAYME_PAYMENT_URL = process.env.PAYME_PAYMENT_URL || "https://payme.uz/home/main";
const CLICK_PAYMENT_URL = process.env.CLICK_PAYMENT_URL || "https://click.uz/";
...
app.post("/order/:id", upload.single("payment_proof"), async (req, res, next) => { const client = await pool.connect(); try {
  ...
  if (paymentType === "online") {
    if (!req.file) throw new Error("Online to'lov uchun чек скриншотини yuklang");
...
function setCartPaymentUI(){const t=(document.querySelector('input[name="payment_type"]:checked')||{}).value||'cash'; const online=t==='online'; document.getElementById('cartOnlinePaymentWrap').style.display=online?'block':'none'; const file=document.getElementById('cartPaymentProofInput'); if(file){file.required=online;} const provider=(cartPaymentProvider&&cartPaymentProvider.value)||'payme'; const link=cartPaymentLinks[provider]||''; if(cartPaymentLinkBtn){cartPaymentLinkBtn.style.display=online&&link?'inline-flex':'none'; cartPaymentLinkBtn.href=link||'#';}}
