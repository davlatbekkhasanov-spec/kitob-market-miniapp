...
function updatePaymentUI(){const t=(document.querySelector('input[name="payment_type"]:checked')||{}).value||'cash'; const online=t==='online'; onlinePaymentWrap.style.display=online?'block':'none'; if(paymentProofInput){ paymentProofInput.required=online; }}
paymentRadios.forEach(x=>x.addEventListener('change',updatePaymentUI)); updatePaymentUI();
...
function setCartPaymentUI(){const t=(document.querySelector('input[name="payment_type"]:checked')||{}).value||'cash'; const online=t==='online'; document.getElementById('cartOnlinePaymentWrap').style.display=online?'block':'none'; const file=document.getElementById('cartPaymentProofInput'); if(file){file.required=online;}}
document.querySelectorAll('input[name="payment_type"]').forEach(x=>x.addEventListener('change',setCartPaymentUI)); setCartPaymentUI();
