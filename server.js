...
function buildPaymentCheckoutLink(provider, amount, orderRef = "") {
  const base = paymentProviderLink(provider);
  if (!base) return "";
  try {
    const u = new URL(base);
    if (Number(amount) > 0) u.searchParams.set("amount", String(Math.round(Number(amount))));
    if (orderRef) u.searchParams.set("comment", String(orderRef));
    return u.toString();
  } catch (_e) {
    return base;
  }
